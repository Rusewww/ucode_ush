...
static int print_exec_error(char *error) {
    if (mx_match(error, "/")) {
...
int mx_exec_command(char **argv, int fd) {
    ...
    } else if (mx_find_command(mx_get_var_val(SHELL, "PATH"), argv[0], &name)) {
        extern char **environ;
        t_process *process = mx_create_process(fd);
        rval = mx_exec(process, name, argv, environ);
        mx_del_process(&process);
        mx_strdel(&name);
    } else {
        rval = print_exec_error(argv[0]);
    }
    return rval;
}
